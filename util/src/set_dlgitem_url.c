...
// --------------------------------------------------------------------------------*/
void SetDlgItemUrl(HWND hdlg, int id, LPTSTR url); 
...
*/    
  ...
  { HGLOBAL hglob = GlobalAlloc(GMEM_MOVEABLE,sizeof(TUrlData));
    ...
    ud->url=HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, (lstrlen(url)+1)*sizeof(TCHAR));/*new char[strlen(url)+1];*/ 
    lstrcpy(ud->url,url);
    ud->hf=0; ud->hb=0;
    GlobalUnlock(hglob);
...
{ HGLOBAL hglob = (HGLOBAL)GetProp(hwnd,"href_dat");
  ...
  { case WM_DESTROY:
    { RemoveProp(hwnd,"href_dat"); GlobalFree(hglob);
      if (ud.url!=0) {} /* FIXME delete[] ud.url;*/
...
{ HGLOBAL hglob = (HGLOBAL)GetProp(hwnd,"href_dlg");
  ...
  { case WM_DESTROY:
    { RemoveProp(hwnd,"href_dlg"); GlobalFree(hglob);
      ...
      HeapFree(GetProcessHeap(), 0, ud.url);
    } break;
